...
with open(specs_yaml_path, 'r') as f:
    ...
    part_spec = all_specs['parts']['P658107YA0A']
...
pitchSpec = part_spec.get('pitchSpec', [20, 111, 124, 216, 95, 140, 20, 726])
idSpec = part_spec.get('idSpec', [0, 0, 0, 0, 0, 0])
tolerance_pitch = part_spec.get('tolerance_pitch', [3.2, 2.2, 2.2, 2.2, 2.2, 2.2, 3.2, 10.0])
...
segmentation_width = part_spec.get('segmentation_width', 1680)

pixelMultiplier = part_spec.get('pixelMultiplier', 0.1650)


def partcheck(image, sahi_predictionList, leftKeypoint, rightKeypoint):
    ...
    if len(detectedposX) > 0:
        ...
        if left_edge is None:
            left_edge = (int(sorted_detections[0].bbox.minx), int(leftmostCenter[1]))
        if right_edge is None:
            right_edge = (int(sorted_detections[-1].bbox.maxx), int(rightmostCenter[1]))
    ...
    elif left_edge is None or right_edge is None:
        status = "NG"
...
def _extract_endpoint_from_keypoints(keypoint_results, x_start, image_width):
    ...
    for kp in keypoint_results:
        try:
            ...
            x_pos, y_pos = kp.keypoints.xy[0, 0].tolist()
